...
window.filterApps = function () {
  const keyword =
    document.getElementById("appSearch")?.value.toLowerCase() || "";
...
import("../features/fileManager.js")
  .then((module) => {
...
// --- SCREEN RECORDING ---
window.startRecordScreen = function () {
  MonitorFeature.startRecording();
};
...
window.startRecordAudio = function () {
  AudioFeature.startRecording();
};

window.cancelRecordAudio = function () {
  AudioFeature.cancelRecording();
};

// DEV: helper to simulate incoming audio file for testing
window._simulateAudioFile = function (base64Payload) {
  // Accept either raw base64 string or an object { payload: base64 }
  AudioFeature.handleAudioDownload(base64Payload);
};
